    ...
    <div className="space-y-10">
      <section>
        ...
        <p className="text-sm text-fg-2 mt-1">
          ...
          <time className="text-fg">{fmtTs(t.ts)}</time>
        </p>
      </section>
      ...
      <section className="flex flex-wrap gap-2 text-sm">
        {sources.map((s) => (
          <Link
            ...
            className="px-3 py-1.5 rounded-md surface hover:bg-surface-2 transition-colors"
          >
            <span className="text-fg mr-2 font-medium">{s.source}</span>
            <span className="text-fg-3 tabular-nums text-xs">
              {s.n.toLocaleString()}
            </span>
          </Link>
        ))}
      ...
      <section className="grid lg:grid-cols-2 gap-8">
        <Card title={tr(loc, "card_top_makes")}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {makes.map((m) => (
                <tr key={m.make} className="border-t border-line">
                  <td className="py-2">
                    <Link
                  ...
                  <td className="text-right tabular-nums text-fg-3 text-xs">
          ...
          <div className="space-y-1.5">
            {years.map((y) => (
              <div key={y.yr4} className="flex items-center gap-3 text-sm">
                <span className="w-12 text-fg-2 tabular-nums">{y.yr4}</span>
            ...
            <tbody>
              {runs.map((r) => (
                <tr key={r.run_id} className="border-t border-line">
                  ...
                  <td className="pl-3 text-fg-3 text-xs">{r.note ?? ""}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="surface rounded-xl p-4">
      <div className="text-[10px] uppercase tracking-[0.08em] font-semibold text-fg-3">
        {label}
      </div>
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="surface rounded-xl p-5">
      <h2 className="text-[11px] font-semibold uppercase tracking-[0.08em] text-fg-3 mb-3">
        {title}
      </h2>
      {children}
    </div>
...
function Empty({ msg }: { msg: string }) {
  return (
    <div className="text-center py-24 text-fg-2">
